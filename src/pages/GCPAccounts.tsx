...
export default function GCPAccounts() {
  ...
  const handleScan = async (account: CloudAccount) => {
    ...
    toast.info("Starting GCP discovery...");
    try {
      const result = await runMultiCloudDiscovery(account.id);
      toast.success(`Discovered ${result.discovered.total} GCP resources!`);
      loadAccounts();
    } catch (error) {
      ...
      <main className="md:ml-64 pt-16">
        <div className="p-6 max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-semibold tracking-tight font-display flex items-center gap-2">🔶 Google Cloud Projects</h1>
