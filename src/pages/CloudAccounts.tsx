...
export default function CloudAccounts() {
  ...
  const [activeProvider, setActiveProvider] = useState<"azure" | "gcp">("azure");
  ...
  const handleConnectAzure = async () => {
    ...
    try {
      ...
      if (error?.code === "23505") {
        toast.error("This Azure subscription is already connected");
      } else {
        toast.error("Failed to connect Azure account");
      }
    } finally {
  ...
  const handleConnectGCP = async () => {
    ...
    try {
      JSON.parse(gcpServiceAccountKey);
    } catch {
    ...
    try {
      ...
      if (error?.code === "23505") {
        toast.error("This GCP project is already connected");
      } else {
        toast.error("Failed to connect GCP project");
      }
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleScan = async (account: CloudAccount) => {
    ...
    toast.info(`Starting ${account.provider.toUpperCase()} discovery...`);
    try {
      const result = await runMultiCloudDiscovery(account.id);
      toast.success(`Discovered ${result.discovered.total} ${account.provider.toUpperCase()} resources!`);
      loadAccounts();
    } catch (error) {
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground">Connect and manage Azure and GCP accounts for multi-cloud security.</p>
            </div>
            <Dialog open={dialogOpen} onOpenChange={(open) => open ? setDialogOpen(true) : resetDialog()}>
              <DialogTrigger asChild>
                <Button className="gap-2"><Plus className="h-4 w-4" /> Connect Cloud Account</Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2"><Cloud className="h-5 w-5 text-primary" /> Connect Cloud Account</DialogTitle>
                  <DialogDescription>Connect your Azure or GCP account for asset discovery.</DialogDescription>
                </DialogHeader>

                <Tabs value={activeProvider} onValueChange={(v) => setActiveProvider(v as "azure" | "gcp")}>
                  ...
                  <TabsContent value="azure" className="space-y-4 mt-4">
                    ...
                    <Button className="w-full" onClick={handleConnectAzure} disabled={isSubmitting}>
                  ...
                  <TabsContent value="gcp" className="space-y-4 mt-4">
                    ...
                    <Button className="w-full" onClick={handleConnectGCP} disabled={isSubmitting}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Card>
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Azure Accounts</p>
                  <p className="text-2xl font-bold">{azureAccounts.length}</p>
                </div>
            ...
            <Card>
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{gcpAccounts.length}</p>
                </div>
                <span className="text-3xl">🔶</span>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-muted-foreground max-w-md mx-auto mb-4">
                  Connect your Azure or GCP accounts to start discovering and securing your multi-cloud infrastructure.
                </p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <span className="text-2xl">{providerIcon(account.provider)}</span>
                        <div>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to disconnect "{accountToDelete?.account_name}"? This will remove the account and its credentials.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
